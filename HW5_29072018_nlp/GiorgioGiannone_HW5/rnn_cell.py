...
class RNNCell(base_layer.Layer):
    ...
    def _rnn_get_variable(self, getter, *args, **kwargs):
        ...
        trainable = variable in tf_variables.trainable_variables() or (
            isinstance(variable, tf_variables.PartitionedVariable)
...
class GRUCell(RNNCell):
    ...
    def call(self, inputs, state):
        ...
        with vs.variable_scope("candidate"):
            ...
            # state = tf.Print([state, tf.shape(state), "state"]
            c = math_ops.tanh(
                _linear([inputs, r * state], self._num_units, True, self._bias_initializer, self._kernel_initializer)
            )
            new_h = (1 - u) * u + u * c
        return new_h, new_h
...
class BasicLSTMCell(RNNCell):
    ...
    def __init__(self, num_units, forget_bias=1.0, state_is_tuple=True, activation=None, reuse=False):
    ...
    def call(self, inputs, state):
        ...
        value = _linear(
            [inputs, h], 4 * self._num_units, True, kernel_initializer=tf.contrib.layers.xavier_initializer()
        )

        i_t, f_t, o_t, c_tilde = array_ops.split(value=value, num_or_size_splits=4, axis=1)

        i_t = math_ops.sigmoid(i_t)
        f_t = math_ops.sigmoid(f_t)  # + self._forget_bias)
        o_t = math_ops.sigmoid(o_t)
        c_tilde = self._activation(c_tilde)

        new_c = f_t * c + i_t * c_tilde
        new_h = o_t * self._activation(new_c)
        ...
        if not state_is_tuple:
            if any(nest.is_sequence(c.state_size) for c in self._cells):
                raise ValueError(
                    ...
                    "state_is_tuple is not set.    State sizes are: %s" % str([c.state_size for c in self._cells])
                )
