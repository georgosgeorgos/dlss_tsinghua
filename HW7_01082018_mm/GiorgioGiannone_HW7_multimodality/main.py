    ...
    for epoch in range(n_epoch):
        ...
        if epoch + 1 == 1 or (epoch + 1) % print_freq == 0:
            if (X_val is not None) and (y_val is not None):
                ...
                for X_val_a, X_val_b, y_val_a in minibatches(X_val, X_val2, y_val, batch_size, shuffle=True):
                    dp_dict = dict_to_one(network.all_drop)  # disable noise layers
...
if __name__ == "__main__":
    ...
    fit(
        ...
        train_op,
        cost,
        np.array(all_train_rgb_samples),
        np.array(all_train_depth_samples),
        ...
        n_epoch=500,
        print_freq=10,
        X_val=np.array(all_test_rgb_samples),
        X_val2=np.array(all_test_depth_samples),
        y_val=np.array(all_test_labels),
        eval_train=True,
    )
