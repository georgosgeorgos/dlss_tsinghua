...
def vae(observed, n, n_x, n_z, n_samples, is_training):
    with zs.BayesianNet(observed=observed) as model:
        ...
        """
        z = zs.Normal("z", z_mean, std=z_std, group_ndims=1).sample(n_samples)
        ...
        x = zs.Normal("x", x_mean, logstd=x_logstd, group_ndims=1)
    return model
    ...
    with zs.BayesianNet(observed=observed) as variational:
        ...
        TODO4: sampling z using the Gaussian distribution of zhusuan
            ...
            > e.g.
                - x = zs.Bernoulli('x', mu, group_event_ndims=1)
        """
        z = zs.Normal("z", z_mean, logstd=z_logstd, group_ndims=1).sample(n_samples)
    ...
    with tf.Session(config=tf.ConfigProto(gpu_options=gpu_options)) as sess:
        ...
        for epoch in range(begin_epoch, epochs + 1):
            ...
            for t in range(iters):
                ...
                _, lb = sess.run(
                    [infer, lower_bound],
                    feed_dict={x: x_batch, learning_rate_ph: learning_rate, n_particles: lb_samples, is_training: True},
                )
                lbs.append(lb)
            time_epoch += time.time()
